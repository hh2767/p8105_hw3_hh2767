...
``` r
ggplot(filter(count, n >= 10000), aes(x = aisle, y = n)) + geom_col(fill = "lightcoral", color = "navy") +  theme(legend.position = "bottom") + scale_y_continuous( limits = c(0, 160000)) + labs(title = "More than 10000 items ordered", y = "number of orders") + coord_flip() + theme(axis.text.x = element_text(face = "plain", color = "black", size = 5))
```
